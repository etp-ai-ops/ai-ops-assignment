...
In this assignment you will revisit CAIDA's Internet Topology Data Kit (ITDK) and learn the Model Context Protocol (MCP), a protocol through which clients discover and call tools. You will turn specific topology questions into restrictive JSON Schema contracts and fixed, parameterized, read-only PostgreSQL queries; test those contracts from schema through an end-to-end MCP call; and use the resulting tools to investigate router-level links while separating observations from uncertain inferences. By the end, you will be able to explain the supplied ITDK relations, implement a deterministic MCP tool boundary, test its behavior, and support analytical claims with captured CSV evidence.

## Overview

- step 1 [read the introduction](Introduction.md)
- step 2 [review the dataset and classroom snapshot](Datasets.md), including the [synthetic fixture notes](data/README.md)
...
- step 7 set up the supplied environment and complete [nids-itdk-mcp.ipynb](nids-itdk-mcp.ipynb)
  - complete each `# YOUR CODE HERE` and `# TODO(student)` region
  - answer all sixteen questions
  - use MCP tools, not direct SQL, for the final investigation
...
If your course handout gives different commands, ports, filenames, or an NRP-specific launch process, the handout takes precedence. Never commit `itdk_mcp_credentials.env`, bearer keys, database passwords, generated CSVs, or licensed ITDK data.
...
├- Task-0-orientation.md                            # setup guidance
├- Task-1-itdk-representation.md                    # ITDK representation guidance
├- Task-2-lookup-tools.md                           # ASN and geolocation guidance
├- Task-3-hostname-selectors.md                     # exclusive-selector guidance
├- Task-4-new-tool.md                               # vertical-slice tool guidance
├- Task-5-topology-investigation.md                 # investigation guidance
├- nids-itdk-mcp.ipynb                          ⬅  # completed, executed notebook
├- src/itdk_mcp/mcp_tools.py                    ⬅  # schemas, validation, dispatch
├- src/itdk_mcp/data_access/nodes.py            ⬅  # ASN/geolocation queries
├- src/itdk_mcp/data_access/hostnames.py        ⬅  # hostname queries
├- src/itdk_mcp/data_access/links.py            ⬅  # new link capability
├- src/itdk_mcp/                                    # supplied server infrastructure
├- tests/test_student_tools.py                  ⬅  # student-authored tests
├- tests/test_completed_infrastructure.py           # supplied infrastructure tests
...
├- itdk_mcp_credentials.env.example                 # configuration names; no secrets
